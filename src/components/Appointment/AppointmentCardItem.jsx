...
const AppointmentCardItem = ({appointment,onDelete}) => {
    ...
    const Date = moment(appointment?.attributes?.date).format('MMMM D')
    const handleDelete = () => {
    ...
    return (
        <View style={{backgroundColor:Colors.white,padding:10,borderWidth:1,borderColor:Colors.light_gray,borderRadius:10,marginBottom:35}}>
            ...
            >{Date} - {appointment?.attributes?.time}</Text>
            ...
            <View style={{display:'flex',flexDirection:'row',gap:10,alignItems:'center'}}>
                ...
                <View>
                    ...
                    >{appointment?.attributes?.doctors?.data[0]?.attributes?.name}</Text>

                    <View style={{display:'flex',flexDirection:'row',gap:5,alignItems:'center',marginTop:5}}>
                        <Ionicons name="location" size={24} color={Colors.primary} />
                        <Text style={{fontSize:15,fontFamily:'roboto',color:Colors.gray}}>{appointment?.attributes?.doctors?.data[0]?.attributes?.address}</Text>
                    </View>

                    <View style={{display:'flex',flexDirection:'row',gap:5,alignItems:'center',marginTop:3}}>
                        <Ionicons name="document-text" size={24} color={Colors.primary} />
                        <Text style={{fontSize:15,fontFamily:'roboto',color:Colors.gray}}>Booking Id: #{appointment?.id}</Text>
                    </View>
                </View>
